...
log=$logDir/$task.log
sample=$(sampleName)
fastq="../../../diamond-viral/$sample/02-map/$task-unmapped.fastq.gz"
out=$task.json.bz2

echo "03-diamond on task $task started at `date`" >> $log
echo "  FASTQ file is $fastq" >> $log

# This should be fixed once we've cleared up some more disk space.
# dbfile=$HOME/scratch/root/share/ncbi/diamond-dbs/bacterial-protein.dmnd
dbfile=$HOME/bacterial-protein.dmnd
